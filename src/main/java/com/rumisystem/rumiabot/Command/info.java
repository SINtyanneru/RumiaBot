					...
					if(US != null){
						...
						if(e.getGuild().getMemberById(US.getId()) != null){
							if(e.getGuild().getMemberById(US.getId()).getNickname() != null){
								...
								eb.setTitle(US.getGlobalName(), null);
							}
						}else{
							eb.setTitle("名前の取得に失敗", null);
						}
						eb.setColor(Main.RUND_COLOR());   //色設定
						eb.setThumbnail(US.getAvatarUrl());

						//eb.addField("この鯖でのニックネーム",e.getGuild().getMember(us).getNickname(), false);
						eb.addField("ID", US.getName(), false);
						if(US.getGlobalName() != null){
							eb.addField("名前", US.getGlobalName(), false);
						}else{
							eb.addField("名前", "なし", false);
						}
						eb.addField("内部ID", US.getId(), false);
						eb.addField("TimeCreated", US.getTimeCreated().toString(), false);
						//BOTか
						if(US.isBot()){
							eb.addField("Ti bot?", "はい", false);
						}else{
							eb.addField("Ti bot?", "いいえ", false);
						}
						//オーナーか
						if(e.getGuild() != null){
							if(e.getGuild().getOwnerId().equals(userId)){
								eb.addField("所有者か", "はい", false);
							}else{
								eb.addField("所有者か", "いいえ", false);
							}
						}else{
							eb.addField("所有者か", "取得失敗", false);
						}
